...
use wisp::{Color, Texture};

const CHECKER_SIZE: u32 = 64;
const CHECKER_CELL: u32 = 8;

fn make_checker(app: &Application) -> Texture {
    let mut bytes = Vec::with_capacity((CHECKER_SIZE * CHECKER_SIZE * 4) as usize);
    for y in 0..CHECKER_SIZE {
        for x in 0..CHECKER_SIZE {
            let cell_x = (x / CHECKER_CELL) % 2;
            let cell_y = (y / CHECKER_CELL) % 2;
            let dark = (cell_x ^ cell_y) == 1;
            let v = if dark { 64 } else { 224 };
            bytes.extend_from_slice(&[v, v, v, 255]);
        }
    }
    Texture::from_rgba(app, CHECKER_SIZE, CHECKER_SIZE, &bytes)
}
...
impl State {
    fn new(event_loop: &ActiveEventLoop) -> Self {
        let window = Arc::new(
            event_loop
                .create_window(Window::default_attributes().with_title("wisp · hello_quad"))
    ...
    fn render(&self) {
        ...
        // Scale checker to ~50% of NDC width (model is in NDC space directly).
        let model = Mat4::from_scale(glam::Vec3::new(0.5, 0.5, 1.0));
