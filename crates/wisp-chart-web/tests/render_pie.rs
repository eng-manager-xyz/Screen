...
use wisp_chart_web::fixtures::pie_fixture;

const W: u32 = 320;
const H: u32 = 320;

#[test]
fn pie_renders_to_snapshot() {
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-chart-book/src/assets/wisp-chart-web/pie.png");
    if let Some(parent) = snapshot.parent() {
        std::fs::create_dir_all(parent).expect("create assets dir");
    }
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8).expect("write pie.png");
}
