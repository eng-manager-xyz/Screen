...
#[test]
fn polar_storyline_hero_renders_to_snapshot() {
    ...
    graphics.container.alpha = 0.8;
    graphics.container.transform = Transform::from_rotation(0.4);
    ...
    assert!(validation.is_none(), "wgpu validation: {validation:?}");
    let bytes = rt.read_pixels(&app);
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-animation-book/src/assets/wisp-animation/composition-hero.png");
    std::fs::create_dir_all(snapshot.parent().unwrap()).unwrap();
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8)
        .expect("write composition-hero.png");
}
