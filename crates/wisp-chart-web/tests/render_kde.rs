...
//! Kernel-density estimate snapshot.
...
#![allow(
    ...
    reason = "viewport bounded by W=360, H=240"
)]
...
use wisp_chart_web::fixtures::kde_fixture;

const W: u32 = 360;
const H: u32 = 240;

#[test]
fn kde_renders_to_snapshot() {
    ...
    let k = kde_fixture();
    let graphics = k.emit_graphics(&theme, viewport);
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-chart-book/src/assets/wisp-chart-web/kde.png");
    if let Some(parent) = snapshot.parent() {
        std::fs::create_dir_all(parent).expect("create assets dir");
    }
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8).expect("write kde.png");
}
