...
#[test]
fn polar_slide_hero_renders_to_snapshot() {
    ...
    graphics.container.transform.position = Vec2::new(0.3, 0.0);
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-animation-book/src/assets/wisp-animation/target-hero.png");
    std::fs::create_dir_all(snapshot.parent().unwrap()).unwrap();
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8)
        .expect("write target-hero.png");
}
