//! Hero for the Multi-Animation / Batching chapter (M-ANIM.20).
//! Polar at alpha=0.7, rotation=1.3 — captures all three batched
//! mutations landing in one frame.

#![cfg(not(target_arch = "wasm32"))]
#![allow(
    clippy::doc_markdown,
    reason = "Variant names in prose without backticks"
)]
#![allow(
...
#[test]
fn batched_hero_renders_to_snapshot() {
    ...
    graphics.container.alpha = 0.7;
    graphics.container.transform = Transform::from_rotation(1.3);
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-animation-book/src/assets/wisp-animation/batched-hero.png");
    std::fs::create_dir_all(snapshot.parent().unwrap()).unwrap();
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8)
        .expect("write batched-hero.png");
}
