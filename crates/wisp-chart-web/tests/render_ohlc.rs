...
#![allow(
    ...
    reason = "viewport bounded by W=480, H=240"
)]
...
use wisp_chart_web::fixtures::ohlc_fixture;

const W: u32 = 480;
const H: u32 = 240;

#[test]
fn ohlc_renders_to_snapshot() {
    ...
    let o = ohlc_fixture();
    let graphics = o.emit_graphics(&theme, viewport);
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-chart-book/src/assets/wisp-chart-web/ohlc.png");
    if let Some(parent) = snapshot.parent() {
        std::fs::create_dir_all(parent).expect("create assets dir");
    }
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8).expect("write ohlc.png");
}
