...
#[test]
fn polar_callbacks_hero_renders_to_snapshot() {
    ...
    graphics.container.alpha = 0.55;
    graphics.container.transform = Transform::from_rotation(1.4);
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-animation-book/src/assets/wisp-animation/lifecycle-hero.png");
    std::fs::create_dir_all(snapshot.parent().unwrap()).unwrap();
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8)
        .expect("write lifecycle-hero.png");
}
