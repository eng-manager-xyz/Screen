...
#[test]
fn polar_spring_hero_renders_to_snapshot() {
    ...
    graphics.container.transform = Transform::from_scale(Vec2::splat(1.08));
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-animation-book/src/assets/wisp-animation/spring-hero.png");
    std::fs::create_dir_all(snapshot.parent().unwrap()).unwrap();
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8)
        .expect("write spring-hero.png");
}
