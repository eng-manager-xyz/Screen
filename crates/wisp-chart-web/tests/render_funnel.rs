...
#![allow(
    ...
    reason = "viewport bounded by W=400, H=300"
)]
...
use wisp_chart_web::fixtures::funnel_fixture;

const W: u32 = 400;
const H: u32 = 300;

#[test]
fn funnel_renders_to_snapshot() {
    ...
    let f = funnel_fixture();
    let graphics = f.emit_graphics(&theme, viewport);
    ...
    let snapshot: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../_docs/wisp-chart-book/src/assets/wisp-chart-web/funnel.png");
    if let Some(parent) = snapshot.parent() {
        std::fs::create_dir_all(parent).expect("create assets dir");
    }
    image::save_buffer(&snapshot, &bytes, W, H, image::ColorType::Rgba8).expect("write funnel.png");
}
