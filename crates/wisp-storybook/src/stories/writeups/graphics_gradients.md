...
The right ellipse uses a **radial gradient** — `t` is the fragment's distance
from `center` divided by `radius`, clamped to `[0, 1]`.

Both gradients evaluate in primitive-local coordinates (`[-half_extents,
+half_extents]`), so the gradient transforms with the primitive's
container — rotate the parent and the gradient rotates too, just like a
painted gradient on a moving cel.
