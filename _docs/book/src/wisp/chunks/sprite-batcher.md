...
100 sprites, all sharing the same texture `Arc`, rendered in a single draw
call.
...
`Texture::id()` is `Arc::as_ptr(&inner) as usize` —
texture-pointer-equality. Cloning a `Texture` shares the GPU resource and
the batch key.

---

[`Sprite` API](../../api/wisp/scene/struct.Sprite.html) · [`Texture`](../../api/wisp/struct.Texture.html) · [Stories index](../stories.md)
