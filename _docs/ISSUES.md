...
- **Description:** The framing inspector exposes 16:9 / 9:16 / 1:1 / 4:3 presets (`framing_inspector.rs`, `ASPECTS`) → `set_aspect` → `EditOp::SetAspect` → `EditProject.aspect` (persisted). But `editor_export.rs` set `EditorPreview::new(...)` and `EncoderConfig` dims from `project.source.width/height` ONLY — `AspectRatio::canvas_dims` had **zero non-test callers** (grep-verified). So picking 9:16/1:1/4:3 mutated+persisted the project but the exported video stayed at the source aspect; the control silently lied. Crop (`CropRect`) IS honored — only the aspect *matte* was unwired.
- **Resolution:** Implemented. `EditProject::canvas_dims()` (pure, in `edit`) reframes the source's longer edge to the project aspect (even dims). `EditorPreview::with_canvas(source_w, source_h, canvas_w, canvas_h)` composes the source *texture* into a differently-shaped *canvas*: a pure `aspect_fit_factor` shrinks the over-long axis so the source keeps its shape (no stretch), the matte area shows the background, and the fit factor is folded into both the screen transform `pad` and the rounded-corner clip / shadow / border window (cursor + zoom follow the same transform). `editor_export` composes + encodes at `canvas_dims` clamped via `fit_within_encoder_limits` (AUT-334 invariants preserved); `editor_preview_session` + the live preview canvas (app-ui) size to the unclamped `canvas_dims` so preview ≡ export, and an aspect change rebuilds the preview session. Tests: `edit::canvas_dims` (Wide/Vertical/Square/Classic + portrait seed), `editor_preview` pure `aspect_fit_factor` (letterbox/pillarbox/unit) + a GPU `with_canvas_letterboxes_a_wide_source_into_a_tall_canvas`, and `editor_export_golden::generator_composes_at_the_aspect_ratio_canvas` (Vertical → 9:16 canvas). A dedicated storybook story/chapter for the matte is a follow-up (the GPU tests are the verification of record). (resolved)
