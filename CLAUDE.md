...
- **CoreFoundation `CFRetained<T>` is `!Send + !Sync` — never store it in a struct that lives in shared Tauri state.** A `RecordingState` field of `Mutex<Option<ClickTap>>` where `ClickTap` holds a `CFRetained<CFRunLoop>` makes the *whole* `RecordingState` `!Send + !Sync` (`Mutex<T>: Sync` needs `T: Send`), and Tauri's `State<T>` requires `Send + Sync` — the compiler then reports a *cascade* of `UnsafeCell<...> cannot be shared/sent between threads` errors plus misleading `no method named X found for State<...>` (the bound failure hides the deref methods). **Fix:** keep the `CFRetained` on the worker thread only (created + dropped inside the spawned closure, never captured/sent), and publish just its address as a `usize` in an `AtomicUsize` (Send+Sync). `CFRunLoopStop` is thread-safe, so the controller thread stops the loop through the stored pointer; the worker holds the run loop alive (blocked in `CFRunLoop::run`) until the stop returns. This is the ISS-16 click-tap pattern (`crates/app/src/click_capture.rs`). Same rule for any `CFRetained<CFMachPort>` / `CFRetained<CFRunLoopSource>` — own them on the FFI worker, share only primitives.
- **`unsafe_code = "warn"` is workspace-wide; a genuine FFI module scopes a `#![allow(unsafe_code, reason = "...")]` at the module top.** `cursor_capture` needed *no* unsafe (objc2's `CGEvent::new` / `location` / `CGDisplayBounds` are safe wrappers); `click_capture`'s `CGEventTapCreate` (raw callback + `user_info`), the `extern "C-unwind"` callback declaration, and the run-loop pointer deref are all genuinely `unsafe`. Put the module-scoped allow inside the `#[cfg(target_os = "macos")] mod imp { #![allow(...)] }` so the non-macOS stub stays unsafe-free, and still document each site's `SAFETY:` invariant. Use `&raw const *retained` (not `&*retained as *const T`) to dodge `clippy::borrow_as_ptr`.
...
- **wisp `Sprite` expects bottom-up BGRA uploads — the flip lives in `RecordingScene::set_*_frame`, NOT at the producer.** The vertex shader maps UV `(0, 0)` to the bottom-left of the rendered NDC quad (the same "+y flip" the sprite vs glyphon bullet flags). A top-down upload renders upside-down. Both SCK (`CVPixelBufferGetBaseAddress`) and GStreamer (`avfvideosrc ! videoconvert ! format=BGRA ! fdsink`) deliver top-down — the universal CoreVideo / GStreamer / Canvas2D convention. **`RecordingScene::set_screen_frame` and `set_camera_frame` accept top-down BGRA and run `flip_bgra_rows_top_down_to_bottom_up` internally before uploading.** Producers keep their bytes in the standard convention so other consumers (`<CameraPreview />` Leptos polls + Canvas2D `putImageData`; a future screen-preview component) can read the shared `FrameSlot` without orientation gymnastics. **When wiring a new video source into the recording pump:** just pass top-down BGRA to `set_*_frame` and let wisp handle the convention. Add a 2-row reversal unit test next to the helper — the planned `s_recording_scene_default` storybook canary was never authored, so unit tests are the only guard. Earlier iterations on `fix-screen-recording` tried producer-side and compose-side flips and both surfaced new bugs the moment a second consumer or a sibling channel changed convention; the wisp-boundary flip is the convergent design.

### Recording pipeline — shared frame / mixer slots

- **Picker-preview pipelines and the recording compose pump must not share unfiltered slots.** Twice now (mic in PR #54, cam in `fix-screen-recording`) a preview-time worker kept writing into a long-lived shared slot (`AudioMixer` / `camera_frame_slot`) on `RecordingState`, and the recording orchestrator handed the same slot to the encoder feed thread — so a disabled channel's preview-time backlog or in-flight frames leaked straight into the recording. Two complementary mitigations: (a) at the worker layer, take `Option<Slot>` at spawn time so the picker preview can pass `None` (the SCK system-audio path does this; the cam worker does not yet — see the deferral in the PROGRESS entry); (b) at the orchestrator layer, hand `start_with_real_capture` a fresh `new_frame_slot()` / `AudioMixer` for any disabled channel so even a still-running preview can't contaminate the recording. Pair (b) with `RecordingScene::set_camera_visible(false)` / `set_screen_visible(false)` driven by `has_*_frame` in `RecordingCompose::compose_frame` so the cam sprite doesn't render against an unwritten `VideoTexture` (wgpu's `create_texture` is not spec'd to zero-init). Whenever a new capture channel lands, audit BOTH layers prophylactically before shipping.
- **Every recorder-owned overlay window must be excluded from SCK capture, or it dups with whatever the compose pump layers on top.** The webcam-bubble Tauri window stayed visible on screen (for user self-monitoring) AND got captured by SCK as part of the screen content AND got composited at `CamLayout::BOTTOM_RIGHT` by wisp → two cam bubbles in the mp4. Fix: thread the bubble's `NSWindow.windowNumber` (== `CGWindowID`) into `ScreenCaptureConfig::excluded_window_ids` so SCK's `initWithDisplay_excludingWindows` drops it. The helper `screen_capture::bubble_window_cg_id` does the resolution via `objc2::msg_send!`. Any future floating overlay (recording-controls HUD, countdown ring, drawing tools, screenshot toolbar) must follow the same pattern — register the window's id in the orchestrator's exclusion list before starting the stream. Window-source captures (`initWithDesktopIndependentWindow`) ignore the exclusion list; only display-source captures need this.
...
- **`just gate` only clippy-checks the native target; CI checks every target it builds for.** `cargo clippy --workspace --all-targets --all-features` operates on the host triple — wasm32-only `cfg(target_arch = "wasm32")` blocks are skipped locally, but CI runs `cargo clippy --target wasm32-unknown-unknown -p wisp-chart-web` separately and any lint there will fail the merge. Before pushing a wasm32-gated change (anything under `crates/wisp-chart-web/src/web.rs`, `crates/app-ui/`, any `#[cfg(target_arch = "wasm32")]` block), run `cargo clippy --target wasm32-unknown-unknown -p <crate> -- -D warnings` locally. Most-common slip-ups caught only on the wasm path: `doc_markdown` violations inside `//!` headers, `manual_split_once` on URL-param parsing, missing `web-sys` feature flags. (Burned 2026-05-14 — `pair.splitn(2, '=')` + an `emit_graphics` non-backticked reference shipped through `just gate` and got caught on macOS+Ubuntu CI clippy runs.)
- **The same blind spot applies across the OS axis, not just `wasm32` — `cfg(not(target_os = "macos"))` bodies never compile on a Mac.** A macOS dev box only ever compiles the `cfg(target_os = "macos")` arm, so a non-macOS stub can ship a `missing_docs` / clippy violation that `just gate` is structurally blind to but `gate-screen (ubuntu/windows)` rejects at `-D warnings`. **When you add a platform `cfg` fork (a `mod imp` split, a `#[cfg(not(target_os = "macos"))]` stub), give the *stub's* `pub` items the same `///` docs + lint hygiene as the real arm** — `missing_docs` is a workspace `warn` lint and the gate denies warnings. (Burned ED.17 — the non-macOS `CursorPoller` stub's `start`/`stop` had no docs; macOS gate-screen was green, ubuntu+windows red on `missing documentation for an associated function`.)
